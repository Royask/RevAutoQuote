...
class GetQuoteScreen : Fragment() {
    ...
    override fun onCreateView(
        ...
        //This loads a basic list based off the deductible of 500 (that's for the whole year)
        val myData = ArrayList<AQuote>().apply { Companies.values().forEach{
            add(it.getQuote(args.client, args.client.vehicle, 950))}}
        ...
        binding.quoteDeductibleRange .addOnChangeListener{ slider:Slider, _: Float, _: Boolean ->
            try {
                ...
                binding.BestQuoteButton.text =
                    quoteAdapter.getFirst()?.run{"Alec's Choice!\n${company.name.replace("_"
                        , " ")}\nPremium: $%,.2f".format(quoteAdapter.getFirst()?.premium)}
